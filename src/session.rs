// The OTA session: what goes on the wire, in what order, and how a write is
// retried until the device acks it. Kept out of main.rs so the CLI file only
// parses arguments and dispatches, and the protocol flow lives in one place
// next to the transport it drives.

use crate::hid::{HidChannel, REPORT_SIZE};
use crate::packet;
use crate::progress::Console;
...
pub fn version(chan: &mut HidChannel, console: &Console) {
...
pub fn flash(chan: &mut HidChannel, path: &str, console: &Console) {
    ...
    while (offset as usize) < total {
        ...
        print!(
            "\r{}",
            crate::progress::render_progress(done, total as u64, started.elapsed().as_secs_f64(), BAR_WIDTH)
        );
